package nobility.handler;

import javafx.stage.FileChooser;
import nobility.handler.modules.Module;
import nobility.handler.modules.hits.HitsExporter;
import nobility.handler.modules.rotmg.RealmOfTheMadGodCore;
import nobility.handler.modules.rotmg.RealmOfTheMadGodCycle;
import nobility.handler.modules.rotmg.RealmOfTheMadGodModule;
import nobility.handler.settings.ComboContainer;
import nobility.handler.settings.ProxyContainer;
import nobility.handler.settings.Settings;
...
public class ModuleHandler {
    ...
    public ModuleHandler(Model model) {
        ...
        if (!resultsFolder.exists()) {
            if (!resultsFolder.mkdir()) {
                Alerter.showError("Couldn't create /data/results/ folder. Please create it manually before starting.");
            }
        }
        ...
        if (!dailySaveFolder.exists()) {
            if (!dailySaveFolder.mkdir()) {
                Alerter.showError("Couldn't create daily save folder. Please create a folder called "
    ...
    public void start() {
        ...
        if (settings.isStopping()) {
            Toast.makeToast("Please wait for the checker to fully stop.");
            return;
        }
        new Thread(() -> {
            if (model.collects().getCombos().isEmpty()) {
                Toast.makeToast("Please load a new combo file first.");
                return;
            }
            if (!module.proxyless()) {
            ...
            if (threads < 5) {
                Toast.makeToast("Threads must be set to 5 and up.");
                return;
            }
            int timeout = model.getTimeouts();
            if (timeout < 1) {
                Toast.makeToast("Timeout must be set to 1 and up.");
                return;
            }
            if (settings.isComplete()) {
                Toast.makeToast("This combo is complete. Please load a new one.");
                return;
            }
            if (currentSaveFolder == null) {
                ...
                if (!currentSaveFolder.exists()) {
                    if (!currentSaveFolder.mkdir()) {
                        Alerter.showError("Couldn't create save folder. There is something wrong with your " +
    ...
    public void stop() {
        ...
        new Thread(() -> {
            ...
            try {
                ...
                model.collects().setComboProgress(settings.getProgress());
    ...
    public void loadComboList() {
        if (settings.isRunning()) {
            Toast.makeToast("Please stop the checker before loading a new combo.");
            return;
        }
        if (settings.isStopping()) {
            Toast.makeToast("Please wait for the checker to fully stop before loading a new combo.");
            return;
        }
        ...
        try {
            ...
            Alerter.showError("Failed to load combo file. Error: " + e.getLocalizedMessage());
            return;
        }
        if (model.collects().getCombos().isEmpty()) {
            Alerter.showError("This combo file is empty. Please load a new one.");
            return;
        }
        if (model.save().getBoolean(Defaults.AUTOREMOVEDUPE_COMBOS)) {
            ...
            if (dedupe.size() == model.collects().getCombos().size()) {
                ...
                Toast.makeToast("Loaded combo file " + settings.getLoadedComboName() + " with "
                        + dedupe.size() + " combos and no duplicates found.");
            } else {
        ...
        model.setComboProgressText(posistion, settings.getComboSize());
        model.updateProgress();
    ...
    public void loadProxyList() {
        if (settings.isRunning()) {
            Toast.makeToast("Please stop the checker before loading a new proxy list.");
            return;
        }
        if (settings.isStopping()) {
            Toast.makeToast("Please wait for the checker to fully stop before loading a new proxy list.");
            return;
        }
        ...
        for (File file : selectedFiles) {
            try (BufferedReader r = new BufferedReader(new FileReader(file))) {
                ...
                Alerter.showError("Failed to load proxy file: " + file.getName() + ". Error: " + e.getLocalizedMessage());
            }
        }
        if (model.collects().getProxies().isEmpty()) {
            Alerter.showError((selectedFiles.size() > 1 ? "These proxy lists are empty. Please load new ones."
        ...
        if (model.save().getBoolean(Defaults.AUTOREMOVEDUPE_PROXIES)) {
            ...
            if (dedupe.size() == model.collects().getProxies().size()) {
                ...
                Toast.makeToast("Loaded proxy file(s) with "
                        + dedupe.size() + " proxies and no duplicates found.");
            } else {
            ...
            Toast.makeToast("Loaded proxy file(s) with " + model.collects().getProxies().size() + " proxies.");
        }
    ...
    public void removeListDuplicates(int list) {
        if (settings.isRunning()) {
            Toast.makeToast("Please stop the checker before removing duplicates.");
            return;
        }
        if (settings.isStopping()) {
            Toast.makeToast("Please wait for the checker to fully stop before removing duplicates.");
            return;
        }
        switch (list) {
            case 0:
                ...
                Set<ComboContainer> dedupe = new LinkedHashSet<>(model.collects().getCombos());
                if (dedupe.size() == model.collects().getCombos().size()) {
                    Toast.makeToast("No combo duplicates found.");
                } else {
                    int dupesFound = model.collects().getCombos().size() - dedupe.size();
                    model.collects().getCombos().clear();
                    model.collects().getCombos().addAll(dedupe);
                    ...
                    model.collects().setComboProgress(posistion);
                    model.setComboProgressText(posistion, settings.getComboSize());
                    Toast.makeToast("Successfully removed " + dupesFound + " duplicate combos.");
                }
                break;
            case 1:
                if (model.collects().getProxies().isEmpty()) {
                ...
                if (dedupe1.size() == model.collects().getProxies().size()) {
                    ...
                    model.setProxies(model.collects().getProxies().size());
                    Toast.makeToast("Successfully removed " + dupesFound + " duplicate proxies.");
                }
                break;
            default:
                break;
    ...
    public void openResultsFolder() {
        new Thread(() -> {
            if (!Desktop.isDesktopSupported()) {
                Toast.makeToast("Desktop is not supported. Unable to open folders on this device.");
                return;
            }
            ...
            try {
                ...
                Alerter.showError("Failed to open folder. Error: " + e.getLocalizedMessage());
            }
        }).start();
    }
