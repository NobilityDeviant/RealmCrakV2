...
public class ProxySettingsController implements Initializable {

    @FXML private Button button_save;
    @FXML private TextField field_threads;
    @FXML private TextField field_timeout;
    @FXML private ComboBox<String> combo_type;
    @FXML private ColorPicker color_elite;
    @FXML private ColorPicker color_anonymous;
    @FXML private ColorPicker color_transparent;
    @FXML private CheckBox auto_remove_check, auto_check_type;
    ...
    public void initialize(URL location, ResourceBundle resources) {
        ...
        if (settings != null) {
            ...
            auto_remove_check.selectedProperty().addListener((observable, oldValue, newValue) -> button_save.setDisable(!settingsChanged()));
            auto_check_type.setSelected(settings.isAutoCheckType());
            auto_check_type.selectedProperty().addListener((observable, oldValue, newValue) -> button_save.setDisable(!settingsChanged()));
        } else {
            Alerter.showAlert(Alert.AlertType.ERROR, "Settings are corrupted",
    ...
    private void button_save_action() {
        ...
        if (timeout < 500) { // restriction due to timeout value being too low will result
            ...
            Alerter.showError("The minimum timeout for requests is 500 milliseconds.");
        } else {
            if (settings != null) {
                ...
                if (ProxySettings.saveConfig(settings)) {
                    Toast.makeToast("The changes made have been saved to the disk!");
                    button_save.setDisable(true);
                } else {
                    Toast.makeToast("Unable to save the configuration file onto the disk!");
                }
            } else {
                Alerter.showAlert(Alert.AlertType.INFORMATION, "Settings are corrupted",
                        "Restart the application before using it.");
            }
        }
    }
